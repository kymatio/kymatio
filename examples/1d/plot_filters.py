...
Original file is located at
    https://colab.research.google.com/drive/1x0VXqMpE2-bmwAwPO78UHkwKJeDHQnF2
"""
...
"""The parameter `J` specifies the maximum scale of the low-pass filters as a power of
two. In other words, the largest filter will be concentrated in a time interval of size `2**J`.
"""
...
We are now ready to plot the frequency response of the filters. We first display the lowpass filter
...
plot to the frequency interval $[0, 0.5]$.
"""
...
plt.title('Frequency response of first-order filters (Q = {})'.format(Q), fontsize=12)
...
The filters have been generated directly in the frequency domain to efficiently compute the convolution using the fft. By applying the ifft we get the filters in the time domain yielding analytic wavelets. We plot the first-order largest wavelet band-pass filter here.
"""
...
plt.ylabel(r'$\psi_1(t)$', fontsize=18)
