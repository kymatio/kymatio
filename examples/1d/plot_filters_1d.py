...
# as parameters. We also give it a filterbank definition that we obtain from
# the `default_filterbank` method. The `scattering_filter_factory` then returns
# the lowpass filter (`phi_f`), the first-order wavelet filters (`psi1_f`), and
# the second-order filters (`psi2_f`).
