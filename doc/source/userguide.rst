...
flexible backend-frontend framework. The backends encapsulate the most computationally
...
For instance, the default backend of ``torch`` frontend, is implemented in PyTorch. This is
...
The backend can be specified via::
    ...
    scattering = Scattering2D(J=2, shape=(32, 32), frontend='torch', backend=backend)
